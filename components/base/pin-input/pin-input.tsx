...
const Slot = ({ index, className, ...props }: ComponentPropsWithRef<"div"> & { index: number }) => {
    ...
    return (
        <div
            ...
            className={cx(
                "bg-primary text-placeholder_subtle shadow-xs ring-primary relative flex items-center justify-center rounded-xl text-center ring-1 ring-inset transition-[box-shadow,background-color] duration-100 ease-linear",
                sizes[size],
                isFocused && slot?.isActive && "ring-brand outline-brand outline-2 outline-offset-2 ring-2",
...
const Separator = (props: ComponentPropsWithRef<"p">) => {
    return (
        <div role="separator" {...props} className={cx("text-display-xl text-placeholder_subtle text-center font-medium", props.className)}>
            -
        </div>
    );
};
...
const Label = ({ className, ...props }: ComponentPropsWithRef<"label">) => {
    ...
    return <label {...props} htmlFor={"pin-input-" + id} id={"pin-input-label-" + id} className={cx("text-secondary text-sm font-medium", className)} />;
};
...
const Description = ({ className, ...props }: ComponentPropsWithRef<"p">) => {
    ...
    return <p {...props} id={"pin-input-description-" + id} role="description" className={cx("text-tertiary text-sm", className)} />;
};
