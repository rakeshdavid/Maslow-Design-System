...
export const DefaultDemo = () => {
    return (
        <Tooltip title="This is a tooltip">
            <TooltipTrigger className="text-fg-quaternary hover:text-fg-quaternary_hover focus:text-fg-quaternary_hover group relative flex cursor-pointer flex-col items-center gap-2 transition duration-100 ease-linear">
    ...
    <div className="grid grid-cols-3 gap-12">
        {PLACEMENTS.map((side, index) => (
            <div key={index} className="flex flex-col items-center justify-center gap-1 text-center">
                ...
                <span className="text-secondary whitespace-nowrap text-xs">{side.label}</span>
            </div>
        ))}
    </div>
);
