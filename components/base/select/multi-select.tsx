                ...
                {(state) => (
                    <div className="flex flex-col gap-1.5">
                        ...
                        <Popover size={"md"} triggerRef={placeholderRef} style={{ width: popoverWidth }} className={props?.popoverClassName}>
                            <AriaListBox selectionMode="multiple" className="outline-hidden size-full">
                                {children}
                            </AriaListBox>
                        </Popover>
        ...
        <div className="relative flex w-full flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
            {!isSelectionEmpty &&
                selectContext?.selectedItems?.items?.map((value) => (
                    <span key={value.id} className="bg-primary pl-1.25 ring-primary flex items-center rounded-md py-0.5 pr-1 ring-1 ring-inset">
                        <Avatar size="xxs" alt={value?.label} src={value?.avatarUrl} />

                        <p className="ml-1.25 text-secondary select-none truncate whitespace-nowrap text-sm font-medium">{value?.label}</p>
            ...
            <div className={cx("relative flex min-w-[20%] flex-1 flex-row items-center", !isSelectionEmpty && "ml-0.5", shortcut && "min-w-[30%]")}>
                <AriaInput
                    ...
                    className="text-md text-primary caret-alpha-black/90 placeholder:text-placeholder focus:outline-hidden disabled:text-disabled disabled:placeholder:text-disabled w-full flex-[1_0_0] appearance-none text-ellipsis bg-transparent outline-none disabled:cursor-not-allowed"
                />
    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled }) =>
                cx(
                    "bg-primary shadow-xs ring-primary outline-hidden relative flex w-full items-center gap-2 rounded-lg ring-1 ring-inset transition duration-100 ease-linear",
