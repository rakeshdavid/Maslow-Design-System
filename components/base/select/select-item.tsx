...
export const SelectItem = ({ label, id, value, avatarUrl, supportingText, isDisabled, icon: Icon, className, children, ...props }: SelectItemProps) => {
    ...
    return (
        <AriaListBoxItem
            ...
            {...props}
            className={(state) => cx("outline-hidden w-full px-1.5 py-px", typeof className === "function" ? className(state) : className)}
        >
            {(state) => (
                <div
                    className={cx(
                        "outline-hidden flex cursor-pointer select-none items-center gap-2 rounded-md",
                ...
                >
                    ...
                    <div className="flex w-full min-w-0 flex-1 flex-wrap gap-x-2">
                        <AriaText
                            slot="label"
                            className={cx("text-md text-primary truncate whitespace-nowrap font-medium", state.isDisabled && "text-disabled")}
                        >
