...
export const CloseButton = ({ label, className, size = "sm", theme = "light", ...otherProps }: CloseButtonProps) => {
    return (
        <AriaButton
            ...
            className={(state) =>
                cx(
                    "focus:outline-hidden flex cursor-pointer items-center justify-center rounded-lg p-2 transition duration-100 ease-linear",
                    sizes[size].root,
                    themes[theme],
        ...
        >
            <CloseIcon aria-hidden="true" className={cx("transition-inherit-all shrink-0", sizes[size].icon)} />
        </AriaButton>
    );
};
