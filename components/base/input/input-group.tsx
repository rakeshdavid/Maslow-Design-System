...
export const InputPrefix = ({ isDisabled, children, ...props }: InputPrefixProps) => (
    <span
        {...props}
        className={cx(
            "text-md text-tertiary shadow-xs ring-border-primary flex ring-1 ring-inset",
    ...
    const paddings = sortCx({
        sm: {
            input: cx(
                ...
                hasTrailing && (prefix ? "group-has-[&>select]:pl-0 group-has-[&>select]:pr-6" : "group-has-[&>select]:pl-3 group-has-[&>select]:pr-6"),
            ),
            leadingText: "pl-3",
        },
        md: {
            ...
            {({ isDisabled, isInvalid, isRequired }) => (
                <>
                    ...
                    <div
                        ...
                        className={cx(
                            "bg-primary group relative flex h-max w-full flex-row justify-center rounded-lg transition-all duration-100 ease-linear",

                            // Only apply focus ring when child is select and input is focused
                            "has-[&>select]:shadow-xs has-[&>select]:ring-border-primary has-[&>select]:has-[input:focus]:ring-border-brand has-[&>select]:has-[input:focus]:ring-2 has-[&>select]:ring-1 has-[&>select]:ring-inset",

                            isDisabled && "has-[&>select]:bg-disabled_subtle has-[&>select]:ring-border-disabled cursor-not-allowed",
