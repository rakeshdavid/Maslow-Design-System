...
const getSizeClasses = (
    ...
    leading: {
        md: {
            root: cx("py-1 pl-1 pr-2 text-xs font-medium", !text && !icon && "pr-1"),
            addon: cx("px-2 py-0.5", theme === "modern" && "gap-1 px-1.5", text && "mr-2"),
            icon: "ml-1 size-4",
        },
        lg: {
            root: cx("py-1 pl-1 pr-2 text-sm font-medium", !text && !icon && "pr-1"),
    ...
    trailing: {
        md: {
            root: cx("py-1 pl-3 pr-1 text-xs font-medium", theme === "modern" && "pl-2.5"),
            addon: cx("py-0.5 pl-2 pr-1.5", theme === "modern" && "pl-2 pr-1.5", text && "ml-2"),
            icon: "ml-0.5 size-3 stroke-[3px]",
            dot: "mr-1.5",
        },
        lg: {
            root: "py-1 pr-1 pl-3 text-sm font-medium",
            addon: cx("py-0.5 pl-2.5 pr-2", theme === "modern" && "pl-2 pr-1.5", text && "ml-2"),
...
const colorClasses: Record<Theme, Record<Color, { root?: string; addon?: string; icon?: string; dot?: string }>> = sortCx({
    ...
    modern: {
        brand: {
            dot: "bg-utility-brand-500 outline-3 outline-utility-brand-100 -outline-offset-1",
        },
        gray: {
            dot: "bg-utility-gray-500 outline-3 outline-utility-gray-100 -outline-offset-1",
        },
        error: {
            dot: "bg-utility-error-500 outline-3 outline-utility-error-100 -outline-offset-1",
        },
        warning: {
            dot: "bg-utility-warning-500 outline-3 outline-utility-warning-100 -outline-offset-1",
        },
        success: {
            dot: "bg-utility-success-500 outline-3 outline-utility-success-100 -outline-offset-1",
        },
    },
});
