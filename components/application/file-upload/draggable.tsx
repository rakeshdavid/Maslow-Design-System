...
export function Draggable({ name, type, size, fileIconType, theme }: DraggableProps) {
    ...
    return (
        <motion.div
            ...
            tabIndex={0}
        >
            <div className="group-focus/drag:bg-tertiary rounded-md p-1.5 group-focus/drag:ring-[0.5px] group-focus/drag:ring-inset group-focus/drag:ring-black/5">
                <FileIcon type={fileIconType || type} variant={theme} className="pointer-events-none size-10" />
            </div>
            <p className="max-w-50 rounded-xs text-primary line-clamp-2 block text-ellipsis text-center text-sm font-medium">
                <span className="rounded-xs group-focus/drag:bg-brand-700 box-decoration-clone px-[3.5px] py-[0.5px] group-focus/drag:text-white group-focus/drag:ring-[0.5px] group-focus/drag:ring-inset group-focus/drag:ring-black/10">
                    {name}
                </span>
            </p>
        </motion.div>
