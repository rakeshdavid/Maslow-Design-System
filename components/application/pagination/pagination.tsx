    ...
    return (
        <Pagination.Item
            ...
            className={({ isSelected }) =>
                cx(
                    "text-quaternary outline-focus-ring hover:bg-primary_hover hover:text-secondary focus-visible:bg-primary_hover flex size-10 cursor-pointer items-center justify-center p-3 text-sm font-medium transition duration-100 ease-linear focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2",
            ...
            <Pagination.Context>
                {({ pages, currentPage, total }) => (
                    <>
                        <div className="hidden justify-center gap-0.5 md:flex">
                            {pages.map((page, index) =>
                                page.type === "page" ? (
                                    ...
                                    <Pagination.Ellipsis key={index} className="text-tertiary flex size-10 shrink-0 items-center justify-center">
                                        &#8230;
                                    </Pagination.Ellipsis>
                                ),
                            )}
                        ...
                        <div className="text-fg-secondary flex justify-center whitespace-pre text-sm md:hidden">
...
export const PaginationCardDefault = ({ rounded, page = 1, total = 10, ...props }: PaginationProps) => {
    ...
    return (
        <Pagination.Root
            ...
            page={page}
            total={total}
            className="border-secondary flex w-full items-center justify-between gap-3 border-t px-4 py-3 md:px-6 md:pb-4 md:pt-3"
        >
            <div className="flex flex-1 justify-start">
                <Pagination.PrevTrigger asChild>
                    <Button iconLeading={ArrowLeft} color="secondary" size="sm">
...
export const PaginationCardMinimal = ({ page = 1, total = 10, align = "left", onPageChange, className }: PaginationCardMinimalProps) => {
    return (
        <div className={cx("border-secondary border-t px-4 py-3 md:px-6 md:pb-4 md:pt-3", className)}>
...
export const PaginationButtonGroup = ({ align = "left", page = 1, total = 10, ...props }: PaginationButtonGroupProps) => {
    ...
    return (
        <div
            className={cx(
                "border-secondary flex border-t px-4 py-3 md:px-6 md:pb-4 md:pt-3",
                            ...
                            {pages.map((page, index) =>
                                page.type === "page" ? (
                                    ...
                                    <Pagination.Ellipsis key={index}>
                                        <ButtonGroupItem className="rounded-none! pointer-events-none size-10 items-center justify-center">
                                            &#8230;
                                        </ButtonGroupItem>
                                    </Pagination.Ellipsis>
                                ),
