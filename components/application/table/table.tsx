...
const TableCardRoot = ({ children, className, size = "md", ...props }: HTMLAttributes<HTMLDivElement> & { size?: "sm" | "md" }) => {
    return (
        <TableContext.Provider value={{ size }}>
            <div {...props} className={cx("bg-primary shadow-xs ring-secondary overflow-hidden rounded-xl ring-1", className)}>
                {children}
            </div>
    ...
    return (
        ...
        >
            {selectionBehavior === "toggle" && (
                <AriaColumn className={cx("relative py-2 pl-4 pr-0", size === "sm" ? "w-9 md:pl-5" : "w-11 md:pl-6")}>
...
const TableHead = ({ className, tooltip, label, children, ...props }: TableHeadProps) => {
    ...
    return (
        <AriaColumn
            ...
            className={(state) =>
                cx(
                    "outline-hidden focus-visible:z-1 focus-visible:ring-focus-ring focus-visible:ring-offset-bg-primary relative p-0 px-6 py-2 focus-visible:ring-2 focus-visible:ring-inset",
            ...
            {(state) => (
                <AriaGroup className="flex items-center gap-1">
                    <div className="flex items-center gap-1">
                        {label && <span className="text-quaternary whitespace-nowrap text-xs font-semibold">{label}</span>}
                    ...
                    {state.allowsSorting &&
                        (state.sortDirection ? (
                            <ArrowDown className={cx("text-fg-quaternary size-3 stroke-[3px]", state.sortDirection === "descending" && "rotate-180")} />
                        ) : (
...
const TableRow = <T extends object>({ columns, children, className, highlightSelectedRow = true, ...props }: TableRowProps<T>) => {
    ...
    return (
        ...
        >
            {selectionBehavior === "toggle" && (
                <AriaCell className={cx("relative py-2 pl-4 pr-0", size === "sm" ? "md:pl-5" : "md:pl-6")}>
...
const TableCell = ({ className, children, ...props }: TableCellProps) => {
    ...
    return (
        <AriaCell
            ...
            className={(state) =>
                cx(
                    "text-tertiary outline-focus-ring focus-visible:z-1 relative text-sm focus-visible:outline-2 focus-visible:-outline-offset-2",
                    size === "sm" && "px-5 py-3",
                    size === "md" && "px-6 py-4",
