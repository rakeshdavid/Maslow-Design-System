...
const FileTypeIcon = ({ type = "folder", theme = "solid", ...props }: FileTypeIconProps) => {
    return (
        <div {...props} className={cx("bg-linear-to-b relative z-10 flex rounded-full from-gray-50 to-gray-200 p-8", props.className)}>
...
const Header = ({ pattern = "circle", patternSize = "md", ...props }: HeaderProps) => {
    ...
    return (
        ...
        >
            {pattern !== "none" && (
                <BackgroundPattern size={patternSize} pattern={pattern} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" />
            )}
            {props.children}
        </header>
    );
...
const Content = (props: ComponentPropsWithRef<"div">) => {
    ...
    return (
        <main
            {...props}
            className={cx(
                "max-w-88 z-10 mb-6 flex w-full flex-col items-center justify-center gap-1",
...
const Description = (props: ComponentPropsWithRef<"p">) => {
    ...
    return <p {...props} className={cx("text-tertiary text-center text-sm", size === "lg" && "text-md", props.className)} />;
};
